...
async function seed() {
    console.log('--- Seeding Final V3 (With Required Columns) ---');
    ...
    for (const s of STUDENTS) {
        ...
        const { data, error } = await supabase
            ...
            .insert({
                ...
                Major: s.major,
                Gender: s.gender,
                University: 'CampusMatch U', // Provided
                Year: 'Junior', // Provided
                Bio: s.bio || 'Hello!',
                PhotoUrl: s.photo,

                // CRITICAL: REQUIRED FIELDS FROM AUTH.TS logic
                PhoneNumber: '',
                InstagramHandle: '',
                LastActive: new Date().toISOString(),
                CreatedAt: new Date().toISOString(),

                // Arrays (Optional but good to have empty if not null)
                Photos: [s.photo],
                Interests: [],
                Prompts: []
            })
    ...
    console.log(`Finished. Created ${count} profiles.`);
}
